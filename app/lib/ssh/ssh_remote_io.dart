...
const Duration _sshTunnelConnectTimeout = Duration(seconds: 8);
const Duration _sshAuthTimeout = Duration(seconds: 15);
const Duration _sshShellTimeout = Duration(seconds: 10);
const Duration _sshCommandTimeout = Duration(seconds: 30);
...
Future<Object?> sshConnect({
  ...
  final sshTargetHost =
      globalSetting.SSHHost.trim().isEmpty
          ? backendHost
          : globalSetting.SSHHost.trim();
  if (backendHost.isEmpty) {
    throw Exception('SSH 后端地址为空，请先连接机器人后端');
  }
  if (username.trim().isEmpty) {
    throw Exception('SSH 用户名为空，请先在设置里保存 SSH 配置');
  }
  final uri = WsChannel.sshTunnelWebSocketUri(backendHost, backendPort);
  final channel = WebSocketChannel.connect(uri);
  _SshOverWsSocket? socket;
  SSHClient? client;
  try {
    await channel.ready.timeout(_sshTunnelConnectTimeout);
    socket = _SshOverWsSocket(channel);
    client = SSHClient(
      socket,
      username: username,
    ...
    await client.authenticated.timeout(_sshAuthTimeout);
    return client;
  } on TimeoutException {
    client?.close();
    await socket?.close();
    ...
    throw Exception(
      'SSH 连接超时：请确认后端 /ws/ssh 可用，且目标 $sshTargetHost:${globalSetting.SSHPort} 的 sshd 已开启',
    );
  } catch (e) {
    client?.close();
    await socket?.close();
...
Future<Object?> sshStartShell(Object? client) {
  ...
  return c
      .shell(pty: const SSHPtyConfig(type: 'xterm'))
      .timeout(_sshShellTimeout);
}
...
Future<String> sshRunRemoteCommand(Object? client, String command) async {
  ...
  final r = await c.runWithResult(cmd).timeout(_sshCommandTimeout);
